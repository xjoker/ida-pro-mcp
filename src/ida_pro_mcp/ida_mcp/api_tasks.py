...
def _cleanup_expired() -> None:
    ...
    with _tasks_lock:
        expired = [
            ...
            and now - t["completed_at"] > _TASK_TTL_SEC
        ]
        for tid in expired:
            del _tasks[tid]
        ...
        try:
            ...
            if resp and "error" in resp:
                with _tasks_lock:
                    ...
                    _tasks[task_id]["error"] = resp["error"].get("message", "unknown error")
                return
            ...
            with _tasks_lock:
                ...
                _tasks[task_id]["result"] = call_result
            ...
            with _tasks_lock:
                _tasks[task_id]["completed_at"] = time.monotonic()
            _cleanup_expired()
